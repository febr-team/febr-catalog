...
load('../febr-data/data/uf.rda')
load("../febr-data/data/febr-chaves.rda")
load("../febr-data/data/dts.rda")
load("../febr-data/data/obs.rda")
```

```{r}
# Ordenar conjuntos de dados alfabeticamente sengundo dataset_id
idx <- match(1:nrow(sheets_keys), order(sheets_keys$ctb))
dts <- dts[order(idx)]
obs <- obs[order(idx)]
sheets_keys <- sheets_keys[order(idx), ]
```
